...
function getMainLoc(configFailed) {
  if (queryString) {
    $.getJSON("https://api.weather.com/v3/location/search?query="+queryString.split("?")[1]+"&language=en-US&format=json&apiKey=" + api_key, function(data) {
      ...
      trafficcoords1.lat = data.location.latitude[0];
      trafficcoords1.lon = data.location.longitude[0];
      trafficcoords2.lat = data.location.latitude[0];
      trafficcoords2.lon = data.location.longitude[0];
      //init data
