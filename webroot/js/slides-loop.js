...
var mainMap
	...
	function showSlides() {
		var currentDisplay, location
			...
			,trafficReport() {
				if (weatherInfo.trafficIncidents.noReport == true) {
					setTimeout(function() {
						$('.info-slide.traffic .noreport').fadeOut(500).promise().done(function(){
							wait(0);
						});
					}, 0);
				} else {
					$('.info-slide-content.traffic-report .thing-top .thingtext').text(thingTopThingText);
					$('.info-slide-content.traffic-report .thing-top .frost-pane.inc1 .incidentdesc1').text(thingTopImpactText + thingTopIncidentDesc);
					$('.info-slide-content.traffic-report .thing-bot .thingtext').text(thingBotThingText);
					$('.info-slide-content.traffic-report .thing-bot .frost-pane.inc2 .incidentdesc2').text(thingBotImpactText + thingBotIncidentDesc);
